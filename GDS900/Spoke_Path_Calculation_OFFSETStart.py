...
def main():
    ...
    # start_angle_deg = 11.25  # Starting angle for the larger wheel

    # # SMALLER WAGON WHEEL
    radius = 39/2  # Radius of the circle in mm
    num_spokes = 8  # Number of spokes
    start_angle_deg = 22.5  # Starting angle for the smaller wheel

    spoke_angle_deg = 360 / num_spokes  # Angle between spokes in degrees

    spoke1_y, spoke1_z = polar_to_cartesian(radius, start_angle_deg)
    ...
    for i in range(2, num_spokes + 1):
        spoke_angle = (start_angle_deg + (i - 1) * spoke_angle_deg) % 360
        ...
        y_steps_dict[f"Spoke 1 to spoke {i}"] = (round(diff_y, 2), y_steps)
        z_steps_dict[f"Spoke 1 to spoke {i}"] = (round(diff_z, 2), z_steps)

    print("Y steps and travel distance (mm):", y_steps_dict)
    print("Z steps and travel distance (mm):", z_steps_dict)
